...
class TrepanClassifierOptimized(RuleTreeClassifier):
        ...
        if node_id is not None and len(str(node_id)) > 1:
            ...
            if parent_node is not None and isinstance(parent_node, TrepanNodeOptimized):
                ...
                if padre_stump is not None:
                    ...
                    # NOTA: con MofNTrepanStumpClassifier, ogni split è una regola M-of-N,
                    # quindi il ramo 'else' (stump univariato) non viene mai eseguito.
                    # Di conseguenza, node_constraints rimane vuoto, ma le regole M-of-N
                    # svolgono lo stesso ruolo di filtro geometrico nel SyntheticDataGenerator.
    ...
    def make_split(self, X, y, X_ts=None, X_img=None, X_txt=None, idx=None, **kwargs):
        if self.max_internal_nodes is not None and hasattr(self, '_internal_nodes_count') and self._internal_nodes_count >= self.max_internal_nodes:  
