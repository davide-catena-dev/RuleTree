...
DummyTree = namedtuple('DummyTree', ['impurity', 'weighted_n_node_samples'])

class MofNTrepanStumpClassifierOptimized(DecisionTreeStumpClassifier):
    ...
    def fit(self, X=None, y=None, X_ts=None, X_img=None, X_txt=None,
            ...
            current_node = getattr(context, '_current_node', None) if context is not None else None
                            
            if hasattr(current_node, 'get_split_data'):
                constraints, m_of_n_rules = current_node.get_split_data()
            else:
            ...
            if context is not None and hasattr(context, '_current_node') and 0 < len(X_local) < S_MIN:
                ...
                local_generator = SyntheticDataGenerator(
                    ...
                    categorical_features=getattr(context, 'categorical', None)
                )
           ...
           # 3. SELEZIONE DEL SEED (split binario - TUTTE le feature sono ammesse)
            ...
            if context is not None and hasattr(context, '_current_node') and context._current_node is not None:
                for m_rule, conds, is_left in m_of_n_rules:
            ...
            if weights_og is not None:
                ...
                n_parent = float(len(y_labels_extended))
            ...
            self.tree_ = DummyTree(impurity=self.impurity,weighted_n_node_samples=[n_parent, n_left, n_right])
    ...
    def node_to_dict(self):
        ...
        rule["is_categorical"] = self.is_categorical
        return rule
    ...
    def dict_to_node(cls, node_dict, X=None):
        ...
        self.is_categorical = node_dict.get("is_categorical", False)
        return self
